...
class GRPOConfig:
    ...
    min_sampling_denoising_std: float = 0.04 # 最小采样方差。RL 探索不能让方差降为0,模型尝试新动作
    ...
    scorer_config: PDMScorerConfig = field(default_factory=lambda: PDMScorerConfig(
        ...
        ttc_weight=5.0,          # 不撞车（Time To Collision），权重适中
...
class ReCogDriveDiffusionPlannerConfig(PretrainedConfig):
    ...
    action_dim: int = 3             # [x, y, heading]
    action_horizon: int = 8         # 预测未来 8 个时间步（通常对应几秒钟的轨迹）
    add_pos_embed: bool = True      # 位置编码
    max_seq_len: int = 8            # 最大序列长度
    ...
    tune_projector: bool = True # 是否微调投影层
    tune_diffusion_model: bool = True # 微调扩散模型
...
class ReCogDriveDiffusionPlanner(nn.Module):
    ...
    def __init__(self, config: ReCogDriveDiffusionPlannerConfig):
        ...
        self.his_traj_encoder = Mlp(
            in_features=12,   # 历史轨迹 [x, y, heading] * 4
            hidden_features=config.hidden_size, #1024
            out_features=config.input_embedding_dim, #1536
        ...
        self.action_encoder = ActionEncoder(
            action_dim=config.action_dim, #3
            hidden_size=config.input_embedding_dim, #1536
        )
        ...
        if config.add_pos_embed:
            self.position_embedding = nn.Embedding(config.max_seq_len, config.input_embedding_dim) # (8, 1536)
    ...
    def sample_time(self, batch_size, device, dtype):
        ...
        if self.config.sampling_method == 'flow':
            sample = self.beta_dist.sample([batch_size]).to(device, dtype=dtype) # (batch_size,), 均为（0,1）之间靠近1
            return (self.config.flow_cfg.noise_s - sample) / self.config.flow_cfg.noise_s # 映射到 [0, 1]，靠近0
        elif self.config.sampling_method in ['ddpm', 'ddim']:
            return torch.randint(0, self.ddpm_num_train_timesteps, (batch_size,), device=device).long() # (batch_size,)，整数时间步0-999
        else:
    ...
    def forward(self, vl_features: torch.Tensor, action_input: BatchFeature) -> BatchFeature:
        ...
        Args:
            ...
            # vl_features (last_hidden_state) 的准确形状为：{[B, 2800, 3584]}. 2800: Token 序列长度（包含文本 Prompt + 图像 Token），3584: 特征维度（假设使用默认的 vlm_size='large'）
            # 数据流：输入图像 & 问题到 RecogDriveBackbone
            # Backbone 处理强制填充到 2800 长度，输出 3584 维特征得到 last_hidden_state。
            # Agent 传递self.action_head(last_hidden_state, ...)
        ...
        ).repeat(1, self.config.action_horizon, 1) #（batch，8，1536）,8 是预测action_horizon
        ego_status_features = self.ego_status_encoder(action_input.status_feature) #（batch，1536）
        
        gt_actions = self.norm_odo(action_input.action) # 归一化后的动作（batch，8，3）

        if self.config.sampling_method == 'flow':
            noise = torch.randn_like(gt_actions)
            t_cont = self.sample_time(gt_actions.shape[0], device=gt_actions.device, dtype=gt_actions.dtype)
            t_cont_reshaped = t_cont[:, None, None] # (batch_size, 1, 1)，由于beta分布这些t靠近0
            
            noisy_actions = (1 - t_cont_reshaped) * noise + t_cont_reshaped * gt_actions
            velocity_target = gt_actions - noise # 标准答案
            t_discrete = (t_cont * self.num_timestep_buckets).long() # 离散化时间（离散时间步）

            action_features = self.action_encoder(noisy_actions, t_discrete) #（batch，8，1536）嵌入了时间步的带噪action
            if hasattr(self, 'position_embedding'):
                pos_ids = torch.arange(action_features.shape[1], device=gt_actions.device)
                action_features += self.position_embedding(pos_ids) # 加位置编码
            
            vl_embeds_mean = vl_embeds.mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1) #（batch，8，1536）,vl特征均值后copy到8个时间步
            fused_input = self.fusion_projector(
                torch.cat((his_traj_features, vl_embeds_mean, action_features), dim=2)
            ) #（batch，8，1536*3）→（batch，8，1536）特征融合
