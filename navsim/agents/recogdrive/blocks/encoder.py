...
class ActionEncoder(nn.Module):
    ...
    def forward(self, actions: torch.Tensor, timesteps: torch.Tensor) -> torch.Tensor:
        ...
        将动作和时间步的信息结合起来，为后续的模型处理提供一个固定大小的嵌入表示
        Returns:
        ...
        if timesteps.dim() == 1 and timesteps.shape[0] == B:
            # shape (B,) => (B,T)
            timesteps = timesteps.unsqueeze(1).expand(-1, T) #将其复制到时间序列的长度
        else:
            raise ValueError(
                "Expected `timesteps` to have shape (B,) so we can replicate across T."
            )

        time_embedding = self.pos_encoding(timesteps).to(dtype=action_embedding.dtype)#为每个时间步生成一个固定的嵌入，用于表示时间信息

        x = torch.cat([action_embedding, time_embedding], dim=-1) #将动作嵌入和时间嵌入连接在一起(B, T, hidden_size * 2)
