...
hydra.initialize(config_path="config/common/train_test_split/scene_filter", version_base=None)
...
# ).to(device)
#----------------------------------------------------------------------------------
# 获取环境变量 (我们在 .sh 里 export 了这些)
checkpoint_path = os.getenv("CHECKPOINT_PATH", "") # 稍后在 sh 里加 export
vlm_path = os.getenv("VLM_PATH", "")               # 稍后在 sh 里加 export

agent = ReCogDriveAgent(
    TrajectorySampling(time_horizon=4, interval_length=0.5),
    checkpoint_path=checkpoint_path, # 使用环境变量
    vlm_path=vlm_path,               # 使用环境变量
    cam_type='single',
    vlm_type='internvl',
    dit_type='small',
    sampling_method='ddim',
    cache_mode=False,                # 注意：这里要是布尔值 False
    cache_hidden_state=False,         # 可视化需要特征，设为 True
    vlm_size='large',                # 注意
    grpo=False,
).to(device)
...
# 修正后的方案 B
metric_cache_loader = MetricCacheLoader(Path('/nfs/dataset-ofs-prediction/rl_lab/leidianqiao/code/recogdrive/exp/metric_cache'))
