...
########################################
# 设置基础路径，默认当前目录，你可以改为绝对路径比如 /Data2/...
...
extract_and_sync_one() {
  ...
  # 使用 rsync 合并目录 (如果用 mv 可能会报错目录已存在)
  rsync -a "$SRC_PATH/" "$SENSOR_DATA_DIR/"
...
if [[ -d "$LOG_DATA_DIR/meta_datas" ]] || [[ -f "$LOG_DATA_DIR/meta_datas.json" ]]; then
    ...
    if download_one "$META_URL" "$META_TGZ"; then
        ...
        if [[ -d "$TMP_DOWNLOAD_DIR/openscene-v1.1/meta_datas" ]]; then
            rsync -a "$TMP_DOWNLOAD_DIR/openscene-v1.1/meta_datas/" "$LOG_DATA_DIR/"
        fi
