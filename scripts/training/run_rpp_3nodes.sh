...
# 3-node SSH launcher for ReCogDrive Stage3 RL (Reinforce++)
# - Verifies /dev/shm=128G on every node
# - Launches worker nodes (rank 1..2) via SSH + nohup
...
# This launcher is intentionally fixed to the first three nodes.
if [[ "${NNODES}" != "3" ]]; then
  echo "[WARN] run_rpp_3nodes.sh forces NNODES=3 (first three nodes)."
  NNODES="3"
fi
...
TMP_BASEDIR="${TMP_BASEDIR:-${RUNTIME_ROOT}/tmp}"
...
RANK0_CUDA_VISIBLE_DEVICES="${RANK0_CUDA_VISIBLE_DEVICES:-}"
RANK1_CUDA_VISIBLE_DEVICES="${RANK1_CUDA_VISIBLE_DEVICES:-}"
RANK2_CUDA_VISIBLE_DEVICES="${RANK2_CUDA_VISIBLE_DEVICES:-}"
...
GRPO_BC_COEFF_START="${GRPO_BC_COEFF_START:-${GRPO_BC_COEFF}}"
GRPO_BC_COEFF_END="${GRPO_BC_COEFF_END:-0.05}"
GRPO_BC_ANNEAL_EPOCHS="${GRPO_BC_ANNEAL_EPOCHS:-${MAX_EPOCHS}}"
...
resolve_cuda_visible_devices() {
  local node_rank="$1"
  case "${node_rank}" in
    0) echo "${RANK0_CUDA_VISIBLE_DEVICES}" ;;
    1) echo "${RANK1_CUDA_VISIBLE_DEVICES}" ;;
    2) echo "${RANK2_CUDA_VISIBLE_DEVICES}" ;;
    *) echo "" ;;
  esac
}
...
if [[ "${QUICK_VALIDATE}" == "1" ]]; then
  ...
  MAX_EPOCHS="1"
  OUTPUT_DIR="${NAVSIM_OUTPUT_ROOT}/recogdrive_rpp_quick_validate_3nodes_${TS}"
fi

echo "=================================================="
echo "🚀 ReCogDrive Stage3 RL (Reinforce++) 3-node SSH"
echo "=================================================="
echo "Nodes:       ${NODES[*]}"
...
echo "Resume ckpt: ${RESUME_CKPT_PATH:-<auto/latest if enabled>}"
...
echo "[STEP] Launch worker ranks 1..$((NNODES-1)) via SSH (nohup)"
for ((node_rank=1; node_rank<NNODES; node_rank++)); do
  node_ip="${NODES[$node_rank]}"
  cuda_visible_devices="$(resolve_cuda_visible_devices "${node_rank}")"
  ...
  ssh "${SSH_OPTS[@]}" "${SSH_USER}@${node_ip}" \
    "MASTER_ADDR='${MASTER_ADDR}' MASTER_PORT='${MASTER_PORT}' NNODES='${NNODES}' GPUS_PER_NODE='${GPUS_PER_NODE}' NODE_RANK='${node_rank}' TS='${TS}' \
     ...
     CACHE_PATH='${CACHE_PATH}' METRIC_CACHE_PATH='${METRIC_CACHE_PATH}' VLM_PATH='${VLM_PATH}' INIT_CHECKPOINT='${INIT_CHECKPOINT}' \
     REFERENCE_POLICY_CHECKPOINT='${REFERENCE_POLICY_CHECKPOINT}' RESUME_CKPT_PATH='${RESUME_CKPT_PATH}' AUTO_RESUME_LATEST='${AUTO_RESUME_LATEST}' OUTPUT_DIR='${OUTPUT_DIR}' \
     RL_ALGO='${RL_ALGO}' MAX_EPOCHS='${MAX_EPOCHS}' EXTRA_OVERRIDES='${EXTRA_OVERRIDES}' \
     CKPT_MONITOR='${CKPT_MONITOR}' CKPT_MODE='${CKPT_MODE}' CKPT_SAVE_TOP_K='${CKPT_SAVE_TOP_K}' \
     CKPT_EVERY_N_EPOCHS='${CKPT_EVERY_N_EPOCHS}' CKPT_SAVE_LAST='${CKPT_SAVE_LAST}' CKPT_FILENAME='${CKPT_FILENAME}' \
     ...
     GRPO_SAMPLE_TIME='${GRPO_SAMPLE_TIME}' GRPO_USE_BC_LOSS='${GRPO_USE_BC_LOSS}' GRPO_BC_COEFF='${GRPO_BC_COEFF}' \
     GRPO_BC_ANNEAL='${GRPO_BC_ANNEAL}' GRPO_BC_COEFF_START='${GRPO_BC_COEFF_START}' \
     GRPO_BC_COEFF_END='${GRPO_BC_COEFF_END}' GRPO_BC_ANNEAL_EPOCHS='${GRPO_BC_ANNEAL_EPOCHS}' \
     GRPO_REFERENCE_KL_COEFF='${GRPO_REFERENCE_KL_COEFF}' \
     ...
     RANK_CUDA_VISIBLE_DEVICES='${cuda_visible_devices}' PYTORCH_CUDA_ALLOC_CONF='${PYTORCH_CUDA_ALLOC_CONF}' \
...
if [[ -n "${RANK_CUDA_VISIBLE_DEVICES:-}" ]]; then
  export CUDA_VISIBLE_DEVICES="${RANK_CUDA_VISIBLE_DEVICES}"
else
...
export PYTORCH_CUDA_ALLOC_CONF
if [[ -n "${RANK0_CUDA_VISIBLE_DEVICES}" ]]; then
  export CUDA_VISIBLE_DEVICES="${RANK0_CUDA_VISIBLE_DEVICES}"
else
  unset CUDA_VISIBLE_DEVICES
fi
